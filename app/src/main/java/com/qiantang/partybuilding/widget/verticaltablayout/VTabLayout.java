...
import com.qiantang.partybuilding.widget.verticaltablayout.widget.TabIndicator;
import com.qiantang.partybuilding.widget.verticaltablayout.widget.TabView;

import java.util.ArrayList;
import java.util.List;

public class VTabLayout extends ScrollView {
    ...
    protected List<OnTabSelectedListener> mTabSelectedListeners;
    private OnTabPageChangeListener mTabPageChangeListener;
    private DataSetObserver mPagerAdapterObserver;
    protected OnItemOnClickListener mListener;

    public VTabLayout(Context context) {
        this(context, null);
    }

    public VTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        setFillViewport(true);
        mTabSelectedListeners = new ArrayList<>();
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.VTabLayout);
...
//        mIndicatorHeight = (int) typedArray.getDimension(R.styleable.VTabLayout_vindicator_height, 100);
        ...
        mTabMode = typedArray.getInteger(R.styleable.VTabLayout_vtab_mode, TAB_MODE_FIXED);
        mTabHeight = (int) typedArray.getDimension(R.styleable.VTabLayout_vtab_height, LayoutParams.WRAP_CONTENT);
        typedArray.recycle();
    }
    ...
    public void setTabAdapter(TabAdapter adapter) {
        ...
        if (adapter != null) {
            ...
            for (int i = 0; i < adapter.getCount(); i++) {
                QTabView tabView = new QTabView(mContext).setIcon(adapter.getIcon(i))
